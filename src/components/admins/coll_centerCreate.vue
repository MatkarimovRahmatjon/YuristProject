  ...
  <div class="min-h-screen flex flex-col items-center p-4">
    ...
    <div v-if="PutModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white p-6 rounded-lg w-11/12 max-w-md">
        ...
        <input v-model="updatedUsername" class="w-full p-2 mb-3 border text-black rounded-lg" placeholder="username" />
    ...
    <div class="flex justify-center mb-16  items-center">
      <h1 style="text-shadow: 0 0 5px #fff,0 0 10px #fff;"
        ...
        Operatorlar ro'yxati <span v-if="Count" class="border-b-4 px-1 border-black text-black">{{ Count }}</span> </h1>
    </div>
      ...
      <div v-for="item in data" :key="item.id"
        ...
        <div @click="path(item.id)" class="relative flex flex-col md:flex-row items-center justify-between p-4 w-full">
          ...
          <div class="ml-4 flex-grow">
            <h1 class="text-2xl font-semibold text-gray-700">Ism: {{ item.name }}</h1>
            <h1 class="text-lg text-gray-500">Username: {{ item.username }}</h1>
            <h1 class="text-base text-gray-500">Role: {{ item.lavozimi }}</h1>
...
const getData = async () => {
  try {
    ...
    const response = await axios.get(`${URL}/coll_center`, {
...
const post = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await axios.post(`${URL}/coll_center`, formData, {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "multipart/form-data",
      },
...
const removeAdmin = async () => {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await axios.delete(`${URL}/coll_center/${selectedId.value}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
onMounted(() => {
  ...
  watch([qwe, PutModal, asd], ([modalOpen, asdOpen, deleteModalOpen]) => {
