<template>
            ...
            <div class="flex items-start max-w-[80%] space-x-2">
              ...
              <div class="flex flex-col space-y-1">
                <div v-if="!isOwnMessage(msg) && msg.sender" class="flex items-center space-x-2">
                  <span v-if="dat === 'datalotin'" class="font-medium text-gray-900">
                    {{ msg.sender?.name }} {{ msg.sender?.surname }} {{ msg.sender?.lavozimi }}
                  </span>
                  <span v-if="dat === 'datakril'" class="font-medium text-gray-900">
                ...
                <div
                  :class="['rounded-3xl px-4 py-4 max-w-sm', isOwnMessage(msg) ? 'bg-blue-500 group hover:bg-lime-500 border-2 rounded-br-none text-white' : 'bg-gray-500 text-white group hover:bg-lime-500 duration-800 border-2 rounded-bl-none']">
                  <div v-if="msg.replyToMessageId" class="text-xs text-gray-300 italic">
                    <h1 class="hidden">
                      {{ getOneMassage(msg.replyToMessageId) }}
                    </h1>
                    {{ mainnncontent }}
                  </div>

                  {{ msg.content }}
                  <div v-if="msg.fileType === 'smiley'">
                    <img :src="URL + `/${msg?.smiley.filePath}`" class="w-40 h-auto rounded-lg" />
                  </div>
                  <div v-if="msg.fileType === 'file' && msg.attachmentUrl" class="border p-2 rounded-lg">
                    <div v-if="/\.(jpg|jpeg|png|gif|webp)$/i.test(msg.attachmentUrl)">
                      <img :src="getMessageImageUrl(msg.attachmentUrl)" class="w-40 h-auto rounded-lg" />
                    </div>
                    <div v-else>
                      <div @click="open(`${URL}${msg.attachmentUrl}`)" target="_blank"
                        class="group-hover:text-blue-500 text-[17px] flex items-center gap-3">
                        <img src="../../../public/file.png" width="40px" alt="">
                        {{ $t('filelarni') }}
                      </div>
                    </div>
                  </div>
                ...
                <div class="text-xs text-gray-500 flex space-x-2">
                  <button @click="handleReply(msg)" class="text-green-500 mx-2">{{ $t('javob') }}</button>
  ...
  <div v-if="showEmojiPicker" class="fixed bottom-[160px] flex gap-2 flex-wrap rounded-md z-50 bg-gray-300 p-6">
    <div v-for="item in smileys" :key="item.id">
      <img width="90px" @click="handleSendMessage('smiley', item.id)" class="block rounded-md"
...
const handleSendMessage = async (type, smileyId) => {
  ...
  if (type === "text") {
    ...
    messageData = {
      senderId,
      content,
      replyToMessageId: replyTo.value || null, // ✅ Backenddagi replyToMessageId ga moslashtirildi
      fileType: null,
      attachmentUrl: null,
      smileyId: smileyId || null, // ✅ Smiley ID ni har qanday holatda jo'natamiz
    };
    newMessage.value = "";
    replyTo.value = null;
  } else if (["file", "audio", "smiley"].includes(type)) {
    const formData = new FormData();
    formData.append("senderId", senderId);
    formData.append("fileType", type);

    if (type === "smiley") {
      formData.append("smileyId", smileyId);
    } else {
      formData.append("file", selectedFile.value);
    }

    formData.append("replyToMessageId", replyTo.value || ""); // ✅ Reply ID ni to'g'ri qo'shildi
...
const getOneMassage = async (id) => {
  ...
  mainnncontent.value = response.data.sender.name
  console.log(mainnncontent.value);
}
