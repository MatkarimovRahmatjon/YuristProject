...
<template>
  <div class="p-6 max-w-3xl mx-auto bg-gradient-to-r from-blue-500 to-purple-500 rounded-xl shadow-lg text-white space-y-4">
    <h2 class="text-2xl font-bold text-center">😊 Smile qo'shish</h2>
    
    <button @click="openUploadModal" class="w-full bg-white text-blue-500 px-4 py-2 z-50 rounded-lg mt-2 font-semibold hover:bg-gray-200 transition">
      + Yangi smiley qo'shish
    </button>

    <div v-if="isUploadModalOpen" class="fixed inset-0 z-50  -top-10 flex items-center justify-center bg-black bg-opacity-50">
      <div class="bg-white p-6 rounded-lg shadow-xl w-96 text-gray-800">
        ...
        <input type="file" @change="handleFileChange" class="block w-full text-sm text-gray-700 border border-gray-300 rounded-lg p-2" />
        
        <div class="flex justify-end space-x-2 mt-4">
          <button @click="closeUploadModal" class="px-4 py-2 bg-gray-400 text-white rounded-lg hover:bg-gray-500">Bekor qilish</button>
          <button @click="uploadFile" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Yuklash</button>
        </div>
    ...
    <div class="mt-6">
      <h3 class="text-lg font-semibold mb-2">🎭 Yuklangan smiley'lar:</h3>
      ...
      <div v-else class="grid grid-cols-4 gap-4">
        <div v-for="smiley in smileys" :key="smiley.id" class="relative bg-white p-2 rounded-lg shadow-md">
          ...
          <img src="../../../public/reject.png" @click="openDeleteModal(smiley)" class="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 w-6 shadow-md"/>
        </div>
      </div>
    </div>
  </div>
  ...
  <div v-if="isDeleteModalOpen" class="fixed inset-0 flex items-center z-50 justify-center bg-black bg-opacity-50">
    <div class="bg-white p-6 rounded-lg shadow-xl w-80 text-center text-gray-800">
      <h3 class="text-2xl font-semibold mb-4 text-black">Haqiqatan ham o‘chirmoqchimisiz?</h3>
      <div class="flex justify-center space-x-2">
        <button @click="closeDeleteModal" class="px-4 py-2 bg-gray-400 text-white rounded-lg hover:bg-gray-500">Bekor qilish</button>
        <button @click="deleteSmiley" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">O‘chirish</button>
      </div>
    </div>
  </div>
</template>