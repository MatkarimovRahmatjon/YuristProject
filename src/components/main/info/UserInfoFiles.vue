<template>
  ...
  <div class="flex mr-[315px] justify-center items-center">
    <h1
        ...
        class="text-black text-[40px] font-bold text-center bg-lime-500 border-[3px] border-black rounded-lg py-2 px-[100px]">Hodim Ma'lumotlari</h1>
  </div>

  <div class="flex absolute right-[22rem] top-[275px] justify-end p-4">
...
const getFiles = async () => {
  ...
  try {
    ...
    files.value = response.data.userFiles.filter(file => file.type === "info");
  } catch (error) {
...
const removeSelectedItem = async () => {info
  try {info
    await axios.delete(`${URL}/userFiles/${deleteItemId.value}`);info
    closeDeleteModal();info
    getFiles();info
  } catch (error) {
